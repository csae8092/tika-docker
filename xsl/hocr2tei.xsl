...
-->
<!-- taken from https://github.com/OCR-D/format-converters/blob/master/hocr2tei.xsl, Apache-2.0 license -->
...
<xsl:variable name="without_extension" select="tokenize($filename1, '\.')[1]"/>
  <xsl:variable name="imagename" select="tokenize(replace($document-uri, '.hocr', '.tif'), 'output/')[last()]"/>
    ...
    <xsl:for-each select="//h:div[@class = 'ocr_page']">
      <xsl:element name="surface">
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:element name="graphic">
              <xsl:attribute name="url" select="$imagename"/>
            </xsl:element>
          </xsl:otherwise>
        </xsl:choose>
...
<xsl:template match="h:div[@class = 'ocr']">
  <xsl:apply-templates/>
</xsl:template>
...
<xsl:template name="at-facs">
  <xsl:if test="starts-with(@title, 'bbox')">
    <xsl:attribute name="facs">#image_<xsl:value-of select="$without_extension"/>_<xsl:value-of select="concat(@class, '_', count(preceding::h:*[@class = current()/@class]))"/>
    </xsl:attribute>
  </xsl:if>
</xsl:template>
